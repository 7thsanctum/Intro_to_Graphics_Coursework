...
void initialise()
{
	...
	if (!eff.addShader("lit_blended.vert", GL_VERTEX_SHADER))		//###############################Load In All the Shader Files#########################
	...
	if (!eff.addShader("spot_light.frag", GL_FRAGMENT_SHADER))  
		exit(EXIT_FAILURE);
	if (!eff.addShader("lit_texture.frag", GL_FRAGMENT_SHADER))  
		exit(EXIT_FAILURE);
	if (!eff.addShader("dissolved.frag", GL_FRAGMENT_SHADER))  
		exit(EXIT_FAILURE);
	if (!eff.create())  
		exit(EXIT_FAILURE);

	if (!star.addShader("starfield.vert", GL_VERTEX_SHADER))
		exit(EXIT_FAILURE);
	if (!star.addShader("starfield.frag", GL_FRAGMENT_SHADER))  
		exit(EXIT_FAILURE);
	if (!star.create())  
		exit(EXIT_FAILURE);
	...
	if (!post_eff->addShader("post_process.vert", GL_VERTEX_SHADER))
		exit(EXIT_FAILURE);	 
	if (!post_eff->addShader("Motion_blur.frag", GL_FRAGMENT_SHADER))  
		exit(EXIT_FAILURE);
	if (!post_eff->create())  
		exit(EXIT_FAILURE);
	...
	if (!post_eff->addShader("post_process.vert", GL_VERTEX_SHADER))
		exit(EXIT_FAILURE);	 
	if (!post_eff->addShader("sharpen.frag", GL_FRAGMENT_SHADER))  
		exit(EXIT_FAILURE);
	if (!post_eff->create())  
		exit(EXIT_FAILURE);
	...
	if (!post_eff->addShader("post_process.vert", GL_VERTEX_SHADER))
		exit(EXIT_FAILURE);	 
	if (!post_eff->addShader("edge.frag", GL_FRAGMENT_SHADER))  
		exit(EXIT_FAILURE);
	if (!post_eff->create())  
		exit(EXIT_FAILURE);
	...
	if (!post_eff->addShader("post_process.vert", GL_VERTEX_SHADER))
		exit(EXIT_FAILURE);	 
	if (!post_eff->addShader("scharredge.frag", GL_FRAGMENT_SHADER))  
		exit(EXIT_FAILURE);
	if (!post_eff->create())  
		exit(EXIT_FAILURE);
	...
	if (!post_eff->addShader("post_process.vert", GL_VERTEX_SHADER))
		exit(EXIT_FAILURE);	 
	if (!post_eff->addShader("blur.frag", GL_FRAGMENT_SHADER))  
		exit(EXIT_FAILURE);
	if (!post_eff->create())  
		exit(EXIT_FAILURE);
	...
	cubemap_texs.push_back("xneg2.png");						// Load in each image for the cube map
	cubemap_texs.push_back("xpos2.png");						
	cubemap_texs.push_back("ypos2.png");						
	cubemap_texs.push_back("yneg2.png");						
	cubemap_texs.push_back("zpos2.png");						
	cubemap_texs.push_back("zneg2.png");							
	...
	scene = loadScene("scene_3.json");							// Load in our scene from the JSON file.
...
void GLFWCALL handleMouseMove(int mouseX, int mouseY)	//Whenever the mouse is moved perform the code inside of here
{
	...
	if( mouseX < midWindowX) fpCam->rotate(-horizMovement/Sensitivity* deltaTime, 0.0f);	// Rotate the FPS camera
	if( mouseX > midWindowX) fpCam->rotate(-horizMovement/Sensitivity * deltaTime, 0.0f);	// Depending on the sensitivity settings
	if( mouseY < midWindowY) fpCam->rotate(0.0f, vertMovement/Sensitivity * deltaTime);		// And depending on the 
	if( mouseY > midWindowY) fpCam->rotate(0.0f, vertMovement/Sensitivity * deltaTime);		// direction of mouse movement
}

void fpsCameraMove(double deltaTime)	// TODO input class
{
	if (glfwGetKey(GLFW_KEY_SPACE)) fpCam->move(glm::vec3(0.0f, moveSpeed, 0.0f) * (float)deltaTime);		//Move UP
	if (glfwGetKey(GLFW_KEY_LCTRL)) fpCam->move(glm::vec3(0.0f, -moveSpeed, 0.0f) * (float)deltaTime);		//and DOWN
	if (glfwGetKey('W'))  fpCam->move(glm::vec3(0.0f, 0.0f, moveSpeed) * (float)deltaTime);					//Forward 
	if (glfwGetKey('S'))  fpCam->move(glm::vec3(0.0f, 0.0f, -moveSpeed) * (float)deltaTime);				//Backward 
	if (glfwGetKey('A'))  fpCam->move(glm::vec3(moveSpeed, 0.0f, 0.0f) * (float)deltaTime);					//Left 
	if (glfwGetKey('D'))  fpCam->move(glm::vec3(-moveSpeed, 0.0f, 0.0f) * (float)deltaTime); 				//Right 
	if (glfwGetMouseButton(GLFW_MOUSE_BUTTON_LEFT)) fpCam->rotate(glm::pi<float>()/4 * deltaTime, 0.0f);	//Turn Left
	if (glfwGetMouseButton(GLFW_MOUSE_BUTTON_RIGHT)) fpCam->rotate(-glm::pi<float>()/4 * deltaTime, 0.0f);	//Turn Right
}
...
void update (double deltaTime)  
{
	...
	if (glfwGetKey(GLFW_KEY_KP_SUBTRACT)) rotateSpeed -= 0.1f;	// Alter rotation speeds of all the objects
	if (glfwGetKey(GLFW_KEY_KP_ADD)) rotateSpeed += 0.1f;

	if (glfwGetKey(GLFW_KEY_KP_0)) numPadSelect = 0;	// Assign a value so that the last
	if (glfwGetKey(GLFW_KEY_KP_1)) numPadSelect = 1;	// Numpad key is remembered
	if (glfwGetKey(GLFW_KEY_KP_2)) numPadSelect = 2;	// Used for changing the focus of the cameras
	if (glfwGetKey(GLFW_KEY_KP_3)) numPadSelect = 3;	// As well as which object is currently being moved
	if (glfwGetKey(GLFW_KEY_KP_4)) numPadSelect = 4;
	if (glfwGetKey(GLFW_KEY_KP_5)) numPadSelect = 5;
	if (glfwGetKey(GLFW_KEY_KP_6)) numPadSelect = 6;
	if (glfwGetKey(GLFW_KEY_KP_7)) numPadSelect = 7;
	if (glfwGetKey(GLFW_KEY_KP_8)) numPadSelect = 8;
	if (glfwGetKey(GLFW_KEY_KP_9)) numPadSelect = 9;

	if (glfwGetKey('1')) camSelect = 1;			// Assign values so that the
	else if (glfwGetKey('2')) camSelect = 2;	// last Number pressed is remembered
	else if (glfwGetKey('3')) camSelect = 3;	// Used for changing the post processing
	else if (glfwGetKey('4')) camSelect = 4;	// Effects
	else if (glfwGetKey('5')) camSelect = 5;
	else if (glfwGetKey('6')) camSelect = 6;
	else if (glfwGetKey('7')) camSelect = 7;
	else if (glfwGetKey('8')) camSelect = 8;

	if (numPadSelect == 1)
	{
		if (glfwGetKey(GLFW_KEY_UP)) scene->objects["sun"]->transform.move(glm::vec3(0.0f, 0.0f, 0.1f));
		if (glfwGetKey(GLFW_KEY_DOWN)) scene->objects["sun"]->transform.move(glm::vec3(0.0f, 0.0f, -0.1f));
		if (glfwGetKey(GLFW_KEY_LEFT)) scene->objects["sun"]->transform.move(glm::vec3(0.1f, 0.0f, 0.0f));
		if (glfwGetKey(GLFW_KEY_RIGHT)) scene->objects["sun"]->transform.move(glm::vec3(-0.1f, 0.0f, 0.0f));		
	}
	else if (numPadSelect == 2)
	{
		if (glfwGetKey(GLFW_KEY_UP)) scene->objects["crate"]->transform.move(glm::vec3(0.0f, 0.0f, 0.1f));
		if (glfwGetKey(GLFW_KEY_DOWN)) scene->objects["crate"]->transform.move(glm::vec3(0.0f, 0.0f, -0.1f));
		if (glfwGetKey(GLFW_KEY_LEFT)) scene->objects["crate"]->transform.move(glm::vec3(0.1f, 0.0f, 0.0f));
		if (glfwGetKey(GLFW_KEY_RIGHT)) scene->objects["crate"]->transform.move(glm::vec3(-0.1f, 0.0f, 0.0f));
	}	
	else if (numPadSelect == 3)
	{
		if (glfwGetKey(GLFW_KEY_UP)) scene->objects["barrel_01"]->transform.move(glm::vec3(0.0f, 0.0f, 0.1f));
		if (glfwGetKey(GLFW_KEY_DOWN)) scene->objects["barrel_01"]->transform.move(glm::vec3(0.0f, 0.0f, -0.1f));
		if (glfwGetKey(GLFW_KEY_LEFT)) scene->objects["barrel_01"]->transform.move(glm::vec3(0.1f, 0.0f, 0.0f));
		if (glfwGetKey(GLFW_KEY_RIGHT)) scene->objects["barrel_01"]->transform.move(glm::vec3(-0.1f, 0.0f, 0.0f));
	}
	...
	if (numPadSelect == 1) orbit = scene->objects["sun"]->transform.position;
	if (numPadSelect == 2) orbit = scene->objects["mercury"]->transform.position;
	if (numPadSelect == 3) orbit = scene->objects["venus"]->transform.position;
	if (numPadSelect == 4) orbit = scene->objects["earth"]->transform.position;
	if (numPadSelect == 5) orbit = scene->objects["mars"]->transform.position;
	if (numPadSelect == 6) orbit = scene->objects["jupiter"]->transform.position;
	if (numPadSelect == 7) orbit = scene->objects["saturn"]->transform.position;
	if (numPadSelect == 8) orbit = scene->objects["uranus"]->transform.position;
	if (numPadSelect == 9) orbit = scene->objects["neptune"]->transform.position;
	if (numPadSelect == 0) orbit = scene->objects["pluto"]->transform.position;

	if (glfwGetKey(GLFW_KEY_KP_DIVIDE)) chsCam->setPositionOffset(chsCam->getPositionOffset() + glm::vec3(0.1f, 0.0f, 0.1f));
	if (glfwGetKey(GLFW_KEY_KP_MULTIPLY)) chsCam->setPositionOffset(chsCam->getPositionOffset() - glm::vec3(0.1f, 0.0f, 0.1f));	
	if (glfwGetKey(GLFW_KEY_KP_ENTER)) rotateSpeed = 0.0f;
	...
	if (glfwGetKey(GLFW_KEY_UP)) lightAim.z -= lightAimSpeed;
	if (glfwGetKey(GLFW_KEY_DOWN)) lightAim.z += lightAimSpeed;	
	if (glfwGetKey(GLFW_KEY_LEFT)) lightAim.x -= lightAimSpeed;
	if (glfwGetKey(GLFW_KEY_RIGHT)) lightAim.x += lightAimSpeed;
	
	fpsCameraMove(deltaTime);

	if (glfwGetKey('I')) chsCam->rotate(glm::vec3(glm::pi<float>() / 100.0f, 0.0f, 0.0f ));
	if (glfwGetKey('K')) chsCam->rotate(glm::vec3(-glm::pi<float>() / 100.0f, 0.0f, 0.0f));	
	if (glfwGetKey('J')) chsCam->rotate(glm::vec3(0.0f, -glm::pi<float>() / 100.0f, 0.0f));	
	if (glfwGetKey('L')) chsCam->rotate(glm::vec3(0.0f, glm::pi<float>() / 100.0f, 0.0f));

	//if (glfwGetKey('1')) scene = loadScene("scene2.json");	// TODO load in alternate scenes

	if (glfwGetKey('[')) glPolygonMode( GL_FRONT, GL_LINE );			// Change to wire frame mode 
	if (glfwGetKey(']')) glPolygonMode( GL_FRONT_AND_BACK, GL_FILL );	// Change to normal mode
	if (glfwGetKey('P')) abCam->rotate(glm::pi<float>() / 100.0f, 0.0f  );	// Rotate the arcball camera
	if (glfwGetKey(';')) abCam->rotate(-glm::pi<float>() / 100.0f, 0.0f );
...
void render()
{
	...
	if(camSelect == 1) post_norm->render(true); 
	else if(camSelect == 2) post_grey->render(true); 
	else if(camSelect == 3) post_blur->render(true); 
	else if(camSelect == 4) pass_2[frame]->render(true); 
	else if(camSelect == 5) post_shar->render(true); 
	else if(camSelect == 6) post_edge->render(true); 
	else if(camSelect == 7) post_sced->render(true); 
	else if(camSelect == 8)
	{
		glViewport (0, 0, resX/2, resY/2);
