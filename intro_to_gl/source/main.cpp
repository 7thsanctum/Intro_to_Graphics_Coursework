...
#pragma comment(                                                               \
    lib,                                                                       \
    "glfw3dll") // Now using GLFWDLL.lib instead of GLFW.lib (compatibilty)
#else
#pragma comment(                                                               \
    lib,                                                                       \
    "glfw3dll") // Now using GLFWDLL.lib instead of GLFW.lib (compatibilty)
...
#pragma comment(lib, "libglew_sharedd") // GLEW library (Allows us to use modern
                                        // openGL functions)
...
bool running = true; // Whether the program is running or not, when set the
                     // false program will terminate!
float resX = 800.0f; // Holder for the screen resolution
float resY =
    600.0f; // Placeholder value incase no resolution is selected somehow
...
float dissolveFactor =
    10.0f;          // The dissolve factor (modified by using - = on keyboard)
...
float rotateSpeed = 1.0f; // The rotation speed of all objects. Can be modified
                          // with + - on numpad
...
glm::vec3 orbit = glm::vec3(
    0.0f, 0.0f, 0.0f); // For use when orbiting around a specific point
glm::vec3 lightAim(0.0f, 0.0f, -500.0f);      // The point the light aims at
glm::vec4 *data = new glm::vec4[1024 * 1024]; // Data for the post process
                                              // effect
...
texture *tex3 =
    new texture(" "); // Our holder for the procedurally generated texture
GLFWwindow *window;

DWORD WINAPI genTexture(LPVOID args) {
  bool check = true; // Switches between the two procedural texture generation
                     // algorithms, I should've put this in texture.cpp
  int texXY = 1024;
  while (running) {
    if (check) {
      for (int i = 0; i < texXY;
           ++i) // TODO add these generation algorithms to texture.cpp
        for (int j = 0; j < texXY; ++j) {
          float f =
              (glm::simplex(glm::vec2((float)i / 160.0f, (float)j / 160.0f)));
...
void initialise() {
  ...
  chsCam = new chase_camera(); //###############################Initialise all
                               // the cameras###########################
  ...
  tarCam->setTarget(glm::vec3(
      0.0f, 2.0f, 0.0f)); //###############################End Camera
                          // Initialise################################

  if (!eff.addShader(
          "assets/shaders/lit_blended.vert",
          GL_VERTEX_SHADER)) //###############################Load In All the
                             // Shader Files#########################
    exit(EXIT_FAILURE);      //###############################And post process
                             // effects#############################
  ...
  if (!post_eff->addShader("assets/shaders/post_process.vert",
                           GL_VERTEX_SHADER))
    exit(EXIT_FAILURE);
  if (!post_eff->addShader("assets/shaders/Motion_blur.frag",
                           GL_FRAGMENT_SHADER))
    exit(EXIT_FAILURE);
  if (!post_eff->create())
    exit(EXIT_FAILURE);
  ...
  if (!post_eff->addShader("assets/shaders/post_process.vert",
                           GL_VERTEX_SHADER))
    exit(EXIT_FAILURE);
  if (!post_eff->addShader("assets/shaders/scharredge.frag",
                           GL_FRAGMENT_SHADER))
    exit(EXIT_FAILURE);
  if (!post_eff->create())
    exit(EXIT_FAILURE);
  ...
  if (!post_eff->addShader("assets/shaders/post_process.vert",
                           GL_VERTEX_SHADER))
    exit(EXIT_FAILURE);
  if (!post_eff->addShader("assets/shaders/post_process.frag",
                           GL_FRAGMENT_SHADER))
    exit(EXIT_FAILURE);
  if (!post_eff->create())
    exit(EXIT_FAILURE);
  ...
  pass_2[1]->create(
      resX, resY); //####################End Shader
                   // Loading##############################################

  texture *tex = new texture(
      "assets/images/star.tga"); //#####################################
  tex->create();                 //#	  Load the texture for the star	  #
  points = new point_sprite(createStarField(2500),
                            tex); //#	and create points for point sprite#
  points->create();               //#####################################

  std::vector<std::string> cubemap_texs; //#####################################
  cubemap_texs.push_back(
      "assets/images/xneg2.png"); // Load in each image for the cube map
  ...
  scene = loadScene(
      "./assets/scenes/scene_3.json"); // Load in our scene from the JSON file.

  CreateThread(0, 0, genTexture, NULL, 0,
               0); // Run the genTexture() on a seperate thread so that it can
                   // be repeatedly updated whilst the program runs but without
                   // affecting performance
}

void handleKeyInput(
    int key,
    int action) // Whenever a key is pressed perform the code inside of here
{
  // if(action == GLFW_PRESS) { }
  // TODO add code to handleKeyInput as soon as a key is pressed
}

void handleMouseMove(GLFWwindow *window, double mouseX,
                     double mouseY) // Whenever the mouse is moved perform the
                                    // code inside of here
{
  float Sensitivity = 10.0f;

  int horizMovement = mouseX - midWindowX; // Find how far from the centre of
                                           // the screen the mouse has moved
  int vertMovement = mouseY - midWindowY;

  glfwSetCursorPos(window, midWindowX,
                   midWindowY); // Reset the mouse position to the centre of the
                                // window each time

  double deltaTime = (currentTimeStamp - prevTimeStamp);
  if (mouseX < midWindowX)
    fpCam->rotate(-horizMovement / Sensitivity * deltaTime,
                  0.0f); // Rotate the FPS camera
  if (mouseX > midWindowX)
    fpCam->rotate(-horizMovement / Sensitivity * deltaTime,
                  0.0f); // Depending on the sensitivity settings
  if (mouseY < midWindowY)
    fpCam->rotate(0.0f, vertMovement / Sensitivity *
                            deltaTime); // And depending on the
  if (mouseY > midWindowY)
    fpCam->rotate(0.0f, vertMovement / Sensitivity *
                            deltaTime); // direction of mouse movement
}
...
void fpsCameraMove(double deltaTime) // TODO input class
{
  ...
  if (glfwGetKey(window, GLFW_KEY_LEFT_CONTROL))
    fpCam->move(glm::vec3(0.0f, -moveSpeed, 0.0f) *
                (float)deltaTime); // and DOWN
  if (glfwGetKey(window, 'W'))
    fpCam->move(glm::vec3(0.0f, 0.0f, moveSpeed) * (float)deltaTime); // Forward
  if (glfwGetKey(window, 'S'))
    fpCam->move(glm::vec3(0.0f, 0.0f, -moveSpeed) *
                (float)deltaTime); // Backward
...
void update(double deltaTime) {
  running =
      ...
      !glfwWindowShouldClose(
          window); // If ESC pressed or program isn't open, stop the program.
  ...
  tex3->create(1024, 1024,
               data); // Update texture with the data that is being calcualated
  ...
  angle =
      angle +
      rotateSpeed *
          deltaTime; // Constantly update the angle so that objects will rotate

  if (glfwGetKey(window, '=')) // Dissolve all the textures
    dissolveFactor =
        glm::clamp<float>(dissolveFactor - 1.0f * deltaTime, 0.0f, 1.0f);
  ...
  //########################################################################################################################################################
  scene->objects["ball_03"]->transform.position =
      glm::vec3(0.0f, -40.0f, -500.0f);
  orbit = scene->objects["ball_03"]->transform.position;
  scene->objects["ball_03"]->transform.rotate(glm::pi<float>() / 4 * deltaTime,
                                              glm::vec3(1.0f, 0.0f, 1.0f));

  scene->objects["ball"]->transform.position =
      glm::vec3(sin(angle) * rad, 0.0f, cos(angle) * rad) + orbit;
  scene->objects["ball"]->transform.rotate(glm::pi<float>() / 4 * deltaTime,
                                           glm::vec3(1.0f, 0.0f, 1.0f));

  // scene->objects["ball_04"]->transform.position = glm::vec3(sin(-angle*8)*3 ,
  // sin(-angle*8)*3 , cos(-angle*4)*3) + orbit; //Cool movement
  scene->objects["ball_04"]->transform.position =
      glm::vec3(sin(-angle * 8) * 3 + 10, -10.0f, cos(-angle * 4) * 3 - 490.0f);
  scene->objects["ball_04"]->transform.rotate(glm::pi<float>() / 4 * deltaTime,
                                              glm::vec3(1.0f, 0.0f, 1.0f));

  orbit = scene->objects["ball"]->transform.position;

  scene->objects["ball_02"]->transform.position =
      glm::vec3(sin(-angle * 4) * 3, sin(-angle * 4) * 3 + cos(-angle * 4) * 3,
                cos(-angle * 4) * 3) +
      orbit;
  scene->objects["ball_02"]->transform.rotate(glm::pi<float>() / 4 * deltaTime,
                                              glm::vec3(1.0f, 0.0f, 1.0f));

  orbit = scene->objects["ball_02"]->transform.position;

  scene->objects["torus"]->transform.position = orbit;
  scene->objects["torus"]->transform.rotate(glm::pi<float>() / 4 * deltaTime,
                                            glm::vec3(1.0f, 0.0f, 1.0f));

  scene->objects["torus_02"]->transform.rotate(
      glm::pi<float>() / 2 * deltaTime,
      glm::vec3(sin(-angle * 4) * 3, cos(-angle * 4) * 3, cos(-angle * 4) * 3));
  scene->objects["torus_03"]->transform.rotate(
      glm::pi<float>() / 2 * deltaTime,
      glm::vec3(sin(-angle * 4) * 3, cos(-angle * 4) * 3, cos(-angle * 4) * 3));
  ...
  scene->objects["sun"]->transform.rotate(glm::pi<float>() / 4 * deltaTime,
                                          glm::vec3(1.0f, 0.0f, 1.0f));

  scene->objects["mercury"]->transform.position =
      glm::vec3(sin(angle) * 12, 0.0f, cos(angle) * 18) + orbit;
  scene->objects["mercury"]->transform.rotate(glm::pi<float>() / 2 * deltaTime,
                                              glm::vec3(1.0f, 0.0f, 1.0f));

  scene->objects["venus"]->transform.position =
      glm::vec3(sin(angle / 2.5) * 27.92, 0.0f, cos(angle / 2.5) * 28.432) +
      orbit;
  scene->objects["venus"]->transform.rotate(-glm::pi<float>() / 2 * deltaTime,
                                            glm::vec3(1.0f, 0.0f, 1.0f));

  scene->objects["earth"]->transform.position =
      glm::vec3(sin(angle / 4.15) * 38.4, 0.0f, cos(angle / 4.15) * 39.728) +
      orbit;
  scene->objects["earth"]->transform.rotate(glm::pi<float>() / 2 * deltaTime,
                                            glm::vec3(1.0f, 0.0f, 1.0f));
  scene->objects["earthmoon"]->transform.position =
      glm::vec3(sin(angle / 0.46) * 0.5218, sin(angle / 0.46) * 0.5218,
                -cos(angle / 0.46) * 0.5218) +
      scene->objects["earth"]->transform.position;
  scene->objects["earthmoon"]->transform.rotate(
      -glm::pi<float>() / 4 * deltaTime, glm::vec3(1.0f, 0.0f, 1.0f));

  scene->objects["mars"]->transform.position =
      glm::vec3(sin(angle / 7.8) * 53.44, 0.0f, cos(angle / 7.8) * 64.96) +
      orbit;
  scene->objects["mars"]->transform.rotate(-glm::pi<float>() / 4 * deltaTime,
                                           glm::vec3(1.0f, 0.0f, 1.0f));
  scene->objects["phobos"]->transform.position =
      glm::vec3(sin(angle / 0.004) * 0.2153728, sin(angle / 0.004) * 0.2153728,
                -cos(angle / 0.004) * 0.2153728) +
      scene->objects["mars"]->transform.position;
  scene->objects["phobos"]->transform.rotate(-glm::pi<float>() / 2 * deltaTime,
                                             glm::vec3(1.0f, 0.0f, 1.0f));

  scene->objects["jupiter"]->transform.position =
      glm::vec3(sin(angle / 49.2) * 193.28, 0.0f, cos(angle / 49.2) * 213.12) +
      orbit;
  scene->objects["jupiter"]->transform.rotate(glm::pi<float>() / 4 * deltaTime,
                                              glm::vec3(1.0f, 0.0f, 1.0f));
  scene->objects["ganymede"]->transform.position =
      glm::vec3(sin(angle / 0.122) * 4.89, sin(angle / 0.122) * 4.89,
                -cos(angle / 0.122) * 4.89) +
      scene->objects["jupiter"]->transform.position;
  scene->objects["ganymede"]->transform.rotate(
      -glm::pi<float>() / 2 * deltaTime, glm::vec3(1.0f, 0.0f, 1.0f));

  scene->objects["saturn"]->transform.position =
      glm::vec3(sin(angle / 122.22) * 339.2, 0.0f,
                cos(angle / 122.22) * 393.92) +
      orbit;
  scene->objects["saturn"]->transform.rotate(-glm::pi<float>() / 2 * deltaTime,
                                             glm::vec3(1.0f, 0.0f, 1.0f));
  scene->objects["titan"]->transform.position =
      glm::vec3(sin(angle / 0.18) * 4.2, sin(angle / 0.18) * 4.2,
                -cos(angle / 0.18) * 4.2) +
      scene->objects["saturn"]->transform.position;
  scene->objects["titan"]->transform.rotate(-glm::pi<float>() / 2 * deltaTime,
                                            glm::vec3(1.0f, 0.0f, 1.0f));
  scene->objects["saturnring"]->transform.position =
      scene->objects["saturn"]->transform.position;
  scene->objects["saturnring"]->transform.rotate(
      glm::pi<float>() / 4 * deltaTime, glm::vec3(1.0f, 0.0f, 1.0f));

  scene->objects["uranus"]->transform.position =
      glm::vec3(sin(angle / 348.68) * 717.36, 0.0f,
                cos(angle / 348.68) * 782.64) +
      orbit;
  scene->objects["uranus"]->transform.rotate(glm::pi<float>() / 4 * deltaTime,
                                             glm::vec3(1.0f, 0.0f, 1.0f));
  scene->objects["titania"]->transform.position =
      glm::vec3(sin(angle / 0.068) * 1.745, sin(angle / 0.068) * 1.745,
                -cos(angle / 0.068) * 1.745) +
      scene->objects["uranus"]->transform.position;
  scene->objects["titania"]->transform.rotate(-glm::pi<float>() / 2 * deltaTime,
                                              glm::vec3(1.0f, 0.0f, 1.0f));

  scene->objects["neptune"]->transform.position =
      glm::vec3(sin(angle / 683.50) * 1160.88, 0.0f,
                cos(angle / 683.50) * 1186.96) +
      orbit;
  scene->objects["neptune"]->transform.rotate(glm::pi<float>() / 4 * deltaTime,
                                              glm::vec3(1.0f, 0.0f, 1.0f));
  scene->objects["triton"]->transform.position =
      glm::vec3(sin(angle / 0.1) * 1.76, sin(angle / 0.1) * 1.76,
                -cos(angle / 0.1) * 1.76) +
      scene->objects["neptune"]->transform.position;
  scene->objects["triton"]->transform.rotate(-glm::pi<float>() / 2 * deltaTime,
                                             glm::vec3(1.0f, 0.0f, 1.0f));

  scene->objects["pluto"]->transform.position =
      glm::vec3(sin(angle / 1028.64) * 1158.24, 0.0f,
                cos(angle / 1028.64) * 1925.2) +
      orbit;
  scene->objects["pluto"]->transform.rotate(-glm::pi<float>() / 2 * deltaTime,
                                            glm::vec3(1.0f, 0.0f, 1.0f));
  scene->objects["charon"]->transform.position =
      glm::vec3(sin(angle / 0.073) * 0.15, sin(angle / 0.073) * 0.15,
                -cos(angle / 0.073) * 0.15) +
      scene->objects["pluto"]->transform.position;
  scene->objects["charon"]->transform.rotate(-glm::pi<float>() / 2 * deltaTime,
                                             glm::vec3(1.0f, 0.0f, 1.0f));
  //########################################################################################################################################################
  ...
  if (glfwGetKey(window, GLFW_KEY_KP_DIVIDE))
    chsCam->setPositionOffset(chsCam->getPositionOffset() +
  ...
  // if (glfwGetKey('1')) scene = loadScene("scene2.json");	// TODO load in
  // alternate scenes
...
void render(const effect *eff, const glm::mat4 &view,
            const glm::mat4 &projection, const render_object *object) {
  CHECK_GL_ERROR
  glm::mat4 mvp = projection * view * object->transform.getTransformMatrix();
  glUniformMatrix4fv(eff->getUniformIndex("modelViewProjection"), 1, GL_FALSE,
                     glm::value_ptr(mvp));
  glm::mat4 mit =
      glm::inverse(glm::transpose(object->transform.getTransformMatrix()));
  glUniformMatrix4fv(eff->getUniformIndex("modelInverseTranspose"), 1, GL_FALSE,
                     glm::value_ptr(mit));
  glUniformMatrix4fv(eff->getUniformIndex("model"), 1, GL_FALSE,
                     glm::value_ptr(object->transform.getTransformMatrix()));

  glm::mat4 scale = glm::scale(glm::mat4(1.0f), object->transform.scale);
  glUniformMatrix4fv(eff->getUniformIndex("scale"), 1, GL_FALSE,
                     glm::value_ptr(scale));
  CHECK_GL_ERROR
  ...
  if (object->geometry->indexBuffer) {
    ...
    glDrawElements(GL_TRIANGLES, object->geometry->indices.size(),
                   GL_UNSIGNED_INT, 0);
...
void sceneRender() {
  ...
  glUniform3fv(eff.getUniformIndex("eyePos"), 1,
               glm::value_ptr(chsCam->getPosition()));
  std::hash_map<std::string, render_object *>::const_iterator iter =
      scene->objects.begin();
  ...
  glUniform3fv(eff.getUniformIndex("eyePos"), 1,
               glm::value_ptr(abCam->getPosition()));
  ...
  glUniform3fv(eff.getUniformIndex("eyePos"), 1,
               glm::value_ptr(tarCam->getPosition()));
  iter = scene->objects.begin();
  for (; iter != scene->objects.end(); ++iter)
  ...
  glUniform3fv(eff.getUniformIndex("eyePos"), 1,
               glm::value_ptr(fpCam->getPosition()));
  iter = scene->objects.begin();
  for (; iter != scene->objects.end(); ++iter)
...
void render() {
  ...
  glBindTexture(GL_TEXTURE_2D,
                pass_2[(frame + 1) % 2]->getFrameBuffer()->getTextureID());
...
bool resolutionSelect() // Sets the resolution of the application
{
  ...
  const GLFWvidmode *mode =
      glfwGetVideoMode(return_struct); // Gets the current desktop resolution
  ...
  do {
    ...
    std::cout << "Please select your resolution\n1) 800x600\n2) 1024x768\n3) "
                 "1280x720\n4) 1920x1080\n5) Native : "
              << width << "x" << height;
    std::cout << "\n\nYour native resolution is " << width << "x" << height
              << " and has been set as the default\n";
    std::cout << "\nPlease press either 1,2,3,4 or 5.\n";
    SELEC = _getche(); // Takes in the users choice instantly without having to
                       // press enter
...
int main() {
  ...
  int check = MessageBox(
      NULL,
      "Welcome! \nWould you like to run this application in full screen mode?",
      "Welcome to Mark's Program!", MB_YESNO | MB_ICONINFORMATION);
  if (check == IDYES) {
  ...
  // glfwOpenWindowHint(GLFW_FSAA_SAMPLES, 8);	//Anti Aliasing effect only
  // seems to work at 16

  if (!resolutionSelect()) // Select resolution
    std::cout << "\nResolution selection failed, you have been set to the "
                 "default resolution\n";
  ...
  {
    currentTimeStamp =
        glfwGetTime(); // Store the current time in "currentTimeStamp"
    update(currentTimeStamp -
           prevTimeStamp); // Subtract prev time from current time to give us
                           // time passed since last frame
    render();              // Then render
    prevTimeStamp =
        currentTimeStamp; // Then set previous frame time to current frame time
